#!/usr/bin/env python3
"""Build a structured fundamentals table from manually confirmed inputs."""
...
FUNDAMENTALS_CONFIG_PATH = ROOT / "config" / "fundamentals.yaml"
OUTPUT_PATH = ROOT / "data" / "processed" / "fundamentals_latest.csv"
...
def read_fundamentals(path: Path = FUNDAMENTALS_CONFIG_PATH) -> list[dict[str, Any]]:
    if not path.exists():
        return []
...
def normalize_row(row: dict[str, Any], fetched_at: str) -> dict[str, Any]:
    ...
    return {
        **row,
        ...
        "fundamentals_status": "ready" if not gaps else "missing_required_fields",
...
def main() -> int:
    ...
    rows = [normalize_row(row, fetched_at) for row in read_fundamentals()]
    write_csv(rows)
